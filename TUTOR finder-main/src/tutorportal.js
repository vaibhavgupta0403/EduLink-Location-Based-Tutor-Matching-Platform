...
//         <form
//           style={{ display: "grid", margin: "100px", border: "5px solid grey" }}
//         >
//           <div className="us1">
//             <h2>
//               {" "}
...
//     let add =
//       await fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/${longitude},${latitude}.json?access_token=pk.eyJ1IjoiY29kZWxvdmVyNDY1NSIsImEiOiJja3l3dXZ1bmswYzY4MnFxaWZuMzB6bzJ2In0.fc8OZYG7p6jsuXHo5domPA
//         `);
//     if (!add.ok) {
...
//                 ARE YOU ALREADY REGISTER AS ANY TUTOR HERE :{" "}
//                 <select name="istutor" onChange={(e) => this.handlechange(e)}>
...
//                   placeholder="place one singal pdf file"
  ...
  render() {
    return (
      <div className="form-container">
        ...
        <form onSubmit={(e) => this.handleSubmit(e)}>
          <select name="istutor" onChange={this.handleInputChange}>
            <option value="no">NO</option>
            <option value="yes">YES</option>
          </select>
          <input
...
class TutorLogin extends React.Component {
  ...
  toggleLoginForm = () => {
    this.setState({ islogin: this.state.islogin === 1 ? 0 : 1 });
  };

  render() {
    return (
      <div className="popup-tutor">
        <div className="button-group">
          <button onClick={this.toggleLoginForm}>
            {this.state.islogin === 1 ? "Register Tutor" : "Login Tutor"}
          </button>
          <Link to="/" className="shift-user-portal">
            Shift to User Portal
          </Link>
        </div>
        {this.state.islogin === 1 ? <TutorLoginform /> : <Tutorregisterform />}
      </div>
    );
  }
}
